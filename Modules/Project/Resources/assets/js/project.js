...
function initializeMyTaskDataTable() {
  ...
  if (typeof my_task_datatable == "undefined" && task_view == "list_view") {
    ...
    typeof my_task_datatable != "undefined" &&
    task_view == "list_view"
  ) {
    my_task_datatable.ajax.reload();
...
function calculateInvoiceTotal() {
  ...
  var discounted_amount = __calculate_amount(
    discount_type,
    discount_amount,
    subtotal
  );
...
function initializeProjectTaskDatatable() {
  var task_view = $("[name='task_view']:checked").val();
  if (
    typeof project_task_datatable == "undefined" &&
    task_view == "list_view"
  ) {
...
KanbanBoard.prototype.initProjectKanban = function (
  boards,
  jKanbanElemSelector
) {
  var _this = this;
...
$("#employee_timelog_report_daterange").on("cancel.daterangepicker", function (
  ev,
  picker
) {
  $("#employee_timelog_report_daterange").val("");
...
$("#project_timelog_report_daterange").on("cancel.daterangepicker", function (
  ev,
  picker
) {
  $("#project_timelog_report_daterange").val("");
  ...
  swal({
    ...
    if (confirmed) {
      $.ajax({
        ...
        dataType: "json",
        url:
          "/project/project-task-comment/" + comment_id + "?task_id=" + task_id,
        success: function (result) {
          if (result.success) {
            toastr.success(result.msg);
