@if ($__is_essentials_enabled)
    <li class="bg-navy treeview {{ in_array($request->segment(1), ['essentials']) ? 'active active-sub' : '' }}">
        ...
        <ul class="treeview-menu">
            ...
            <li
                class="{{ $request->segment(2) == 'document' && $request->get('type') != 'memos' ? 'active active-sub' : '' }}">
            ...
            <li
                class="{{ $request->segment(2) == 'document' && $request->get('type') == 'memos' ? 'active active-sub' : '' }}">
                <a
                    href="{{ action([\Modules\Essentials\Http\Controllers\DocumentController::class, 'index']) . '?type=memos' }}">
            ...
            @can('edit_essentials_settings')
                <li class="{{ $request->segment(2) == 'settings' ? 'active active-sub' : '' }}">
                    <a
                        href="{{ action([\Modules\Essentials\Http\Controllers\EssentialsSettingsController::class, 'edit']) }}">
