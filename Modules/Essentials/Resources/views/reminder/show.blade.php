        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    @if (!empty($reminder->end_time))
                        {{ @format_time($reminder->end_time) }}
                    @endif
                </div>
            </div>
            <br>
            ...
            <div class="row">
                <div class="col-md-9">
                    {!! Form::open([
                        'url' => action([\Modules\Essentials\Http\Controllers\ReminderController::class, 'update'], [$reminder->id]),
                        'method' => 'PUT',
                        'id' => 'update_reminder_repeat',
                    ]) !!}
                    <div class="input-group">

                        <input type="hidden" id="update_action_url" value="/essentials/reminder/{{ $reminder->id }}">

                        <!-- /btn-group -->
                        {!! Form::select('repeat', $repeat, $reminder->repeat, ['class' => 'form-control', 'required']) !!}
                        <div class="input-group-btn">
                            <button type="submit"
                                class="btn btn-primary change_reminder_repeat">@lang('messages.update')</button>
                        </div>
                    </div>
                    {!! Form::close() !!}
                </div>
                <div class="col-md-3">
                    <button type="button" class="btn btn-danger" id="delete_reminder"
                        data-href="{{ action([\Modules\Essentials\Http\Controllers\ReminderController::class, 'destroy'], [$reminder->id]) }}">
