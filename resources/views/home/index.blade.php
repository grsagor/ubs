...
@section('content')
    ...
    <section class="content-header content-header-custom">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h3 style="flex: 1; color: white;">{{ __('home.welcome_message', ['name' => Session::get('user.first_name')]) }}
            </h3>

            <button id="copyButton" class="btn btn-success">
                <i class="fa fa-copy" aria-hidden="true"></i>
                Copy Customer Register Link
            </button>
        </div>
    </section>
...
@section('javascript')
    ...
    <script>
        document.getElementById('copyButton').addEventListener('click', function() {
            var textToCopy = '{{ route('customer.getRegister', $business_id) }}';
