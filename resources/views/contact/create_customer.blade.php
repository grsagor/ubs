<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
            ...
            <div class="row">
                <div class="col-md-12 mb-12">
                    <div class="form-check">
                        <input type="checkbox" name="select_from_existing_user" class="input-icheck form-check-input"
                            id="select_from_existing_user">
                        <label class="form-check-label" for="select_from_existing_user">Select from existing
                            user?</label>
                    </div>
                </div>
            </div>
            <div id="from_existing_user" style="display: none;">
                <select class="form-control select2" id="existing_user_id">
                    <option value="">Please Select</option>
                    @foreach ($no_customers as $no_customer)
                        <option value="{{ $no_customer->id }}">
                            {{ $no_customer->first_name . ' ' . $no_customer->last_name }}</option>
                    @endforeach
                </select>
            </div>
            <div id="not_from_existing_user">
                <div class="row">
                ...
                @if (!empty($module_form_parts))
                    @foreach ($module_form_parts as $key => $value)
                        @if (!empty($value['template_path']))
                            ...
                            @endphp
                            @include(
                                'crm::contact_login.partial.customer_contact_form_part',
                                $template_data)
                        @endif
                    @endforeach
                @endif
        ...
        <div class="modal-footer">
            <button type="submit" id="not_from_existing_user_btn" class="btn btn-primary">@lang('messages.save')</button>
            <button type="button" id="from_existing_user_btn" style="display: none;" class="btn btn-primary">Save
...
<script>
    $(document).ready(function() {
        $(document).ready(function() {
            $('#select_from_existing_user').on('ifChecked', function() {
                $('#not_from_existing_user').hide();
                $('#from_existing_user').show();
                $('#not_from_existing_user_btn').hide();
                $('#from_existing_user_btn').show();
            }).on('ifUnchecked', function() {
                $('#not_from_existing_user').show();
                $('#from_existing_user').hide();
                $('#not_from_existing_user_btn').show();
                $('#from_existing_user_btn').hide();
            });
            $(document).on('click', '#from_existing_user_btn', function() {
                const user_id = $('#existing_user_id').val();
                // contact_modal
                $.ajax({
                    url: "{{ route('customer.add.from.existing.user') }}", // Replace with your URL
