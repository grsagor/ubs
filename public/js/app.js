$(document).ready(function () {
  ...
  $("#recruitmentTable").DataTable({
    ...
    columnDefs: [
      {
        orderable: false,
        targets: 10, // Disable ordering on the Action column
      },
    ],
  });

  $("#applicants_job_Table").DataTable({
    paging: true,
    lengthChange: true,
    searching: true,
    ordering: true,
    ...
    columnDefs: [
      {
        orderable: false,
        targets: 8, // Disable ordering on the Action column
      },
    ],
  });
  ...
  $(".location_add_modal, .location_edit_modal").on("shown.bs.modal", function (
    e
  ) {
  ...
  if ($("#expense_date_range").length == 1) {
    $("#expense_date_range").daterangepicker(dateRangeSettings, function (
      start,
      end
    ) {
  ...
  $(document).on("change", ".payment_types_dropdown", function () {
    ...
    if (
      $(this)
        .closest(".payment_row")
        .find(".enable_cash_denomination_for_payment_methods").length
    ) {
...
$(document).on("submit", "form#pay_contact_due_form", function (e) {
  ...
  var denomination_for_payment_types = JSON.parse(
    $(
      "#pay_contact_due_form .enable_cash_denomination_for_payment_methods"
    ).val()
  );
  if (
    ...
    var payment_amount = __read_number(
      $("#pay_contact_due_form .payment_amount")
    );
